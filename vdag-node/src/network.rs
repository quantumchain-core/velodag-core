...
use crate::peer_guard::{ConnectionDecision, PeerGuard};
...
fn handle_gossip_block(
    ...
    // expensive part. This also counts as a violation: a well-behaved
    // peer never needs to send something this large.
    if message.data.len() > MAX_GOSSIP_MESSAGE_BYTES {
        ...
        if peer_guard.record_violation(propagation_source) {
            warn!(peer = %propagation_source, "Peer banned after repeated violations");
    ...
    let incoming_block: VeloBlock = match bincode::deserialize(&message.data) {
        Ok(b) => b,
        Err(_) => {
            warn!(peer = %propagation_source, "Dropped malformed block payload from peer");
            if peer_guard.record_violation(propagation_source) {
                warn!(peer = %propagation_source, "Peer banned after repeated violations");
                let _ = swarm.disconnect_peer_id(propagation_source);
            }
...
/// Records a violation against `source` (if known) and disconnects the
/// peer if this pushes them over the ban threshold. A free function rather
/// than a closure over `swarm`/`peer_guard` deliberately -- a capturing
/// closure would hold an exclusive borrow of both for its entire lifetime,
/// which would conflict with the later recursive orphan-replay call that
/// also needs to pass `swarm` and `peer_guard` through. Each call here
/// reborrows fresh instead, the same proven-safe pattern used everywhere
/// else `swarm` is threaded through this file.
fn record_violation_and_maybe_ban(
    swarm: &mut Swarm<VeloBehaviour>,
    peer_guard: &mut PeerGuard,
    source: Option<libp2p::PeerId>,
) {
    if let Some(peer) = source {
        if peer_guard.record_violation(peer) {
            warn!(%peer, "Peer banned after repeated violations");
...
///
/// `source` is the peer this block is attributed to, for violation
/// tracking (see `record_violation_and_maybe_ban`) -- `None` for blocks
/// this node mined itself. Orphaning and duplicate-detection are NOT
/// treated as violations: both are normal, routine network behavior (a
/// block arriving before its parent under ordinary propagation jitter is
/// expected, not evidence of misbehavior), and treating them as violations
/// would ban well-behaved peers under nothing more than ordinary network
/// timing.
///
...
fn validate_and_ingest(
    ...
    if storage.load_block(&hash)?.is_some() {
        return Ok(()); // already known, nothing to do -- not a violation
    }
    ...
    if block.transactions.len() > MAX_TRANSACTIONS_PER_BLOCK {
        ...
        record_violation_and_maybe_ban(swarm, peer_guard, source);
        return Ok(());
    }
    ...
    //    if not -- NOT a violation, see the function doc comment), and once
    //    parents are confirmed known: height must be exactly one more than
    //    the tallest parent (no gaps, no claiming a height that doesn't
    //    follow from the DAG), and timestamp must not precede the tallest
    //    parent's timestamp nor claim to be from more than
    //    MAX_FUTURE_DRIFT_SECS in the future. The future-drift check
    ...
    for parent in &block.header.parents {
        match ghostdag.block_store.get(parent) {
            ...
            None => {
                ...
                orphans.insert(*parent, source, block);
                return Ok(());
            }
        }
    }
    ...
    *current_difficulty_target =
    ...
    // it. They go back through this same validate_and_ingest path, checked
    // against their own height's expected target and attributed to their
    // own original sender (not the sender of the block that happened to
    // resolve the wait) rather than being ingested blind.
    let ready = orphans.take_ready(&ingested_hash);
    for (orphan_source, orphan) in ready {
        validate_and_ingest(
            orphan,
            orphan_source,
            swarm,
    ...
    match response {
        SyncResponse::Blocks(blocks) => {
            ...
            for block in blocks {
                ...
                // orphan handling. Attributed to `peer` -- a peer that
                // sends invalid blocks via sync is just as bannable as one
                // that sends them via gossip.
                validate_and_ingest(
                    block,
                    Some(peer),
                    swarm,
