...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Fixed P2P listen port, for a real deployment (cloud VM, always-on
    // node) where you need a stable, known port for firewall rules and a
    // bootstrap address that doesn't go stale every restart. Defaults to 0
    // (OS-assigned random port), preserving today's local-dev behavior --
    // this is purely opt-in.
    let p2p_port: u16 = env::var("VDAG_P2P_PORT")
        .ok()
    ...
    let mut peer_guard = PeerGuard::new();
