...
impl VeloBlock {
    ...
    pub fn calculate_subsidy_split(height: u64) -> (u64, u64) {
        ...
        // Rust's `>>` on a primitive integer does NOT panic on an
        // out-of-range shift amount in release builds -- it silently masks
        // the shift amount to the type's bit width instead (e.g. shifting
        // by 65 on a u64 behaves like shifting by 1). Left unguarded, an
        // extremely large height (era >= 64) would make this function
        // return a wrong, nonzero reward instead of correctly reporting
        // that the reward has fully halved away to nothing -- the same
        // "silently wrong in release, not even a clean crash" failure mode
        // as the difficulty-adjustment overflow bug fixed earlier. era=64
        // is roughly 250,000 years of continuous 1-second block production,
        // so this is not an urgent practical concern, but the guard is
        // cheap and the alternative is a genuine correctness bug sitting
        // latent in the code.
